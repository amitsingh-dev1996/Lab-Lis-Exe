...
namespace VSoftLIS_Interface
{
    ...
    public partial class VSoftLISMAIN : Form
    {
        private static VSoftLISMAIN MainFormSelfReference = null;
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            try
            {
                CachedData.UpdateDescriptiveResultMaster(analyzer.instrumentid, analyzer.instrumentgroupid);
                CachedData.UpdateValueLimits();

                msgConfig = new MessageConfiguration(analyzer.instrumentid);

                CachedData.UpdateTestList(analyzer.instrumentid);
                ScheduleMethodInThread(() => { CachedData.UpdateTestList(analyzer.instrumentid); }, TimeSpan.FromMinutes(5));
                ...
                {
                    this.Text += " (Other Server connection)";
                }
                ...
                //&& (connectionSettings_WO.TCP_PortNumber == null && String.IsNullOrEmpty(connectionSettings_WO.Serial_PortName)))//added condition of WO to not mix different modes of result and WO
                {
                    ...
                    {
                        if (analyzer.instrumentgroupid == analyzerTypeId_KingfisherPCR)
                        {
                            CachedData.Analyzer.instrumentgroupid =
                                analyzer.instrumentgroupid = analyzerTypeId_QuantStudioManual;
                            msgConfig = new MessageConfiguration(analyzer.instrumentid);
                        }
                    }
                    ...
                    if (msgConfig.IsManualTestSelection)
                    {
                        ...
                        cmbTestSelection.Items.AddRange(CachedData.TestList.Select(r => r.testcode).OrderBy(r => r).ToArray());
                        ResetTestSelection();
        ...
        private void AttemptConnection()
        {
            ...
            {
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Both connections (TCP & Serial) are available, using TCP for communication");
            }
;
            ...
            else if (ports.Count > 0)
            {
                ...
                if (!String.IsNullOrEmpty(connectionSettings_Result.Serial_PortName) && ports.Contains(connectionSettings_Result.Serial_PortName))
                {
                    ...
                    {
                        UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, connectionSettings_Result.Serial_PortName + " is configured in settings, but not available in connected ports. Kindly connect desired port or correct the port in settings.");
                        return;
                    }
            ...
            {
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "No connection available, kindly check Serial Port / TCP Setting, and restart application");
            }
        ...
        {
            if (msgConfig.IsHL7 == true)
            {
                MessageLogger.LogCommunication(data, analyzer.instrumentname);
                try
                {
                    ...
                    sender.ProcessResponseHL7(analyzer, data);
                }
                catch (Exception ex)
                {
                    UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, ex.Message.ToString() /*+ "\n"*/);
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (data == Characters.NAK || data.Contains("\u0015") && msgConfig.AnalyzerTypeID == AnalyzerTypes.Bechman)
                        data = "";

                    //if (data.Contains("DB01") || data.Contains("RB01")||data.Contains("DE01") || data.Contains("RE01") && msgConfig.AnalyzerTypeID == AnalyzerTypes.Bechman)
                    //    data = "";
                    if (data.Contains("Rh") && msgConfig.AnalyzerTypeID == AnalyzerTypes.Bechman)
                        data = data.Replace("Rh", "R ");
        ...
        private void btnOpenPort_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Port " + _communication.ToString() + " opened at " + DateTime.Now /*+ "\n"*/);
                    if (connectionSettings_WO.TCP_PortNumber.HasValue && !IsSamePortForBulkWO && _communication_BulkWO.IsOpen)
                    {
                        UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Port " + _communication_BulkWO.ToString() + " opened at " + DateTime.Now /*+ "\n"*/);
                    }
            ...
            {
                UiMediator.LogAndShowError(analyzer.instrumentid, ex);
            }
        }
        ...
        private void btnClosePort_Click(object sender, EventArgs e)
        {
            if (_communication.IsOpen)
            {
                ...
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Port " + _communication.ToString() + " closed at " + DateTime.Now /*+ "\n"*/);
            }
            if (connectionSettings_WO.TCP_PortNumber.HasValue && !IsSamePortForBulkWO && _communication_BulkWO.IsOpen)
            {
                _communication_BulkWO.ClosePort();
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Port " + _communication_BulkWO.ToString() + " closed at " + DateTime.Now /*+ "\n"*/);
            }
        ...
        {
            dsGridRecords = new DataSet("VSoftLIS_Interface");
        ...
        private void UpdateUI()
        {
            while (true)
            {
                try
                {
                    ...
                    UpdateUiGridMessages(UiMediator.GetUiGridData(analyzer.instrumentid).AsEnumerable());
                }
                catch (Exception ex)
                {
                    errorLog.err_insert(ex);
                        ...
                        try
                        {
                            ...
                            if (msgConfig.ShowPatientDemographics && drGrid["PatientInfo"] is BarcodeList)
                            {
                                ...
                                if (patientInfo != null)
                                {
                                    ...
                                    dr["PatientName"] = patientInfo.customername;
                                    dr["Age"] = patientInfo.age;
                                    dr["Gender"] = patientInfo.gender;
                                    dr["PatientId"] = patientInfo.investigationid;
        ...
        {
            MessageProcessor messageProcessor_ForUploadResults = new MessageProcessor(analyzer.instrumentid);
            int resultsCount = 0;
            ...
            UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Outgoing, resultsCount + " results uploaded.");
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    else if (connectionSettings_Result.ConnectionType == ConnectionType.FileConnected)
                    {
                        ...
                        switch (analyzer.instrumentgroupid)
                        {
                            case 11: //TOSOH G8
                            case 62: //TOSOH G11
                                ...
                                if (dtPending != null && dtPending.Rows.Count > 0)
                                {
                                    try
                                    {
                                        messageProcessor = new MessageProcessor(analyzer.instrumentid);
                                        foreach (DataRow drPending in dtPending.Rows)
                                        {
                                            ...
                                            {
                                                worklist worklist = messageProcessor.GetWorklist(dtPending.AsEnumerable().Select(r => r.Field<string>("Barcode")).ToList(), 3);
                                                messageProcessor.AddResultRecord(barcode, "HbA1C", Convert.ToDecimal(drPending["Element5"]), "", "", (DateTime)drPending["DateTime"], showInGrid: true, patientInfo: worklist.samples.Where(r => r.barcode.Equals(barcode)).SingleOrDefault());
                                            }
                                            else
                                            {
                                        ...
                                        if (!msgConfig.UserSelectionToUploadResults)
                                        {
                                            ...
                                            UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Outgoing, "Results batch uploaded.");
                                        }
        ...
        private void UploadFile(string fileFullPath)
        {
            ...
            try
            {
                ...
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Started file " + FilePath);

                if (msgConfig.InstrumentType == InstrumentTypes.Analyzer)
                {
                    MessageProcessor messageProcessor = new MessageProcessor(analyzer.instrumentid);
                    ...
                    else
                    {
                        ...
                        {
                            UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Warning, "File is with invalid extension " + FilePath);
                            return;
                        }
                        ...
                        if (msgConfig.IsManualTestSelection)
                        {
                            this.Invoke(new Action(() =>
                            {
                                if (cmbTestSelection.SelectedIndex == -1)
                                    UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Select desired test code");
                            }));
                        ...
                        {
                            UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "Could not load file data: " + FilePath);
                            return;
                        }
                        ...
                        for (int i = 0; i < dtImportedData.Rows.Count; i++)
                        {
                            for (int j = 0; j < dtColumnsInfo.Rows.Count; j++)
                            {
                                ...
                                {
                                    if (msgConfig.RemoveUnmappedTestcodes && !CachedData.TestList.Where(r => r.instrumentcode.Equals(strValue, StringComparison.InvariantCultureIgnoreCase)).Any())
                                    {
                        ...
                        if (!isInputUnpivotFormat)
                        {
                            var unmappedTestcodes = dtImportedData.Columns.Cast<DataColumn>().Skip(unpivotHeaderColumns).Select(r => r.ColumnName)
                                .Except(CachedData.TestList.Select(r => r.instrumentcode));

                            if (unmappedTestcodes.Any())
                            {
                                validationErrorCount += unmappedTestcodes.Count();
                                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "Mapping for these Machine Testcode does not exist: " + String.Join(", ", unmappedTestcodes));
                            }
                        }
                        ...
                        if (cellValidationErrors.Any())
                        {
                            ...
                            {
                                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "Row " + err.Key.Key + " Column " + err.Key.Value + " : " + err.Value);
                            }
                        }

                        if (validationErrorCount > 0)
                        {
                            UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, validationErrorCount + " validation error(s) found.");
                            dtImportedData = null;
                        }
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (analyzer.instrumentgroupid == analyzerTypeId_AgilentIcpms)
                            ...
                            worklist worklist = messageProcessor.GetWorklist(dtImportedData_Unpivot.AsEnumerable().Select(r => r.Field<string>("Barcode")).Distinct().ToList(), 3);
                            foreach (DataRow drPending in dtImportedData_Unpivot.Rows)
                            {
                                ...
                                machineTestCode = analyzer.instrumentgroupid == analyzerTypeId_BiotekReader ? machineTestCode : (string)drPending["TESTCODE"];
                                ...
                                decimal testValue = 0;
                                int descriptiveId = -1;
                                var product_DescriptiveList = messageProcessor.GetDescriptiveMapping(tli.testcode);
                                if (product_DescriptiveList.Any())
                                {
                                    //descriptiveId = Convert.ToInt32(strTestValue);
                                    descriptiveId = messageProcessor.PopulateDescriptiveId(tli.testcode, strTestValue);
                                    testValue = -1;
                                }
                                else
                                {
                                ...
                                messageProcessor.AddResultRecord(barcode, machineTestCode, testValue, "", "", resulted_Time, descriptiveId: descriptiveId, showInGrid: true, patientInfo: worklist.samples.Where(r => r.barcode.Equals(barcode)).SingleOrDefault());
                            }
                ...
                else if (msgConfig.InstrumentType == InstrumentTypes.Archival)
                {
                    ...
                    UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Barcode : " + Barcode + " Message : " + response.Message);
                }
            ...
            {
                string newFileDirectory = Path.Combine(VSoftLISMAIN.ApplicationDataFolder, targetFolderName, DateTime.Now.Date.ToString("yyyy-MM-dd"));
        ...
        private DataTable ImportExcel(string FileName, string extention, bool hasHeaders)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(strConn))
            {
                ...
                {
                    UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "Sheet \"" + excelSheetName + "\" does not exist in excel file.");
                    return null;
                }
            ...
            {
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "The file uploaded does not have minimum number of rows required. Please check if you have uploaded correct file.");
                return null;
            }
            ...
            for (int i = 0; i < dtColumnsInfo.Rows.Count; i++)
            {
                ...
                if (headerRowNumber > 1 && analyzer.instrumentgroupid != analyzerTypeId_KingfisherPCR)
                {
                    ...
                    {
                        UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "Column \"" + strColumnInfo + "\" is missing or does not match defined position. Please upload data in predefined format.");
                        return null;
                    }
                }
            }

            if (analyzer.instrumentgroupid == analyzerTypeId_Navios)
            {
            ...
            else if (analyzer.instrumentgroupid == analyzerTypeId_KingfisherPCR)
            {
                ...
                foreach (var bg in barcodesGroup)
                {
                   ...
                   && componentValue < minvalue && componentValue != 0)
                    ...
                    if (kitCode == "N gene" || kitCode == "Meril")
                    {
                        ...
                        if (analyzer.instrumentid == 2045 || analyzer.instrumentid == 2046)
                        {
                    ...
                    if (kitCode == "TATA_MD")
                    {
                        string strTestValue = new MessageProcessor(analyzer.instrumentid).GetDescriptiveMapping("COVTM").Where(r => r.resultid == testValue).SingleOrDefault()?.result;
                    ...
                    {
                        string strTestValue = new MessageProcessor(analyzer.instrumentid).GetDescriptiveMapping(Testselected).Where(r => r.resultid == testValue).SingleOrDefault()?.result;
                    ...
                    {
                        string strTestValue = new MessageProcessor(analyzer.instrumentid).GetDescriptiveMapping("COV19").Where(r => r.resultid == testValue).SingleOrDefault()?.result;
                        dtImportedData.Rows.Add(barcode, "COV19", strTestValue, "", resultedTime);
                    }

                    if (testValue != (int)DescriptiveIDs.NotDetected && analyzer.instrumentid != 2045 && analyzer.instrumentid != 2046)
                    {
                        if (kitCode == "N gene" || kitCode == "Meril")
                        {
            ...
            using (StreamReader sr = new StreamReader(fileName))
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (Path.GetExtension(fileName) == ".txt")
                    {
                        ...
                        if (analyzer.instrumentgroupid == analyzerTypeId_LCMS)
                        {
                        ...
                        else if (analyzer.instrumentgroupid == analyzerTypeId_BiotekReader)
                        {
                    ...
                    else
                    {
                        ...
                        if (analyzer.instrumentgroupid == analyzerTypeId_RotorGene)
                        {
                            if (cntRow == 4)
                        ...
                        if (analyzer.instrumentgroupid == analyzerTypeId_BDFacts)
                        {
                        ...
                        if (cntRow == (headerRowNumber) && analyzer.instrumentgroupid != analyzerTypeId_BDFacts)
                        {
                            //TODO: remove following line
                            //copy testcode columns
                        ...
                        if (analyzer.instrumentgroupid == analyzerTypeId_RotorGene)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Warning, "No result value for barcode " + barcode);
                                    continue;
                                }
                                ...
                                int testcodeId = CachedData.TestList.Where(r => testCode.Equals(r.testcode, StringComparison.InvariantCultureIgnoreCase)).Single().testid;
                        ...
                        else if (analyzer.instrumentgroupid == analyzerTypeId_BDFacts)
                        {
                            ...
                            if (!IsHb27)
                            {
                                ...
                                for (int i = 115; i < unpivotHeaderColumns; i++)
                                {
                                    ...
                                    string _testCode = CachedData.TestList.Where(r => mac.Equals(r.instrumentcode, StringComparison.InvariantCultureIgnoreCase)).Single().instrumentcode;
                                    decimal testvalue = 0;
                ...
                {
                    UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "Header row not found at predefined position. Please check if you have uploaded correct file.");
                }
            }
            return dtImportedData;
        }
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			System.Diagnostics.Process.Start("C:\\ProgramData\\VSoftLIS\\ApplicationData\\" + analyzer.instrumentname + "\\Logs\\");
        ...
        private void PerformBulkWorklistOperations()
        {
            ...
            foreach (int action in new int[] { /*0,*/ 1, 2 })
            {
                ...
                worklist wList = messageProcessor.GetWorklist(new List<string> { "ALL" }, action);
                int barcodeCount = wList == null ? 0 : wList.samples.Count;
                ...
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Fetched " + barcodeCount + " barcodes WO.");
                ...
                {
                    foreach (BarcodeList bList in wList.samples)
                    {
                        worklist wl = new worklist();
                        wl.samples = new List<BarcodeList>();
                        wl.samples.Add(bList);

                        records = new MessageProcessor(analyzer.instrumentid).PrepareWorklistMessages(new List<string> { bList.barcode }, wl?.samples, isCancellation);
                ...
                {
                    int batchSize = 100;
                    for (int cntr = 0; cntr < wList.samples.Count(); cntr += batchSize)
                    {
                        //bList = wList.barcodeList.Skip(batchNumber * batchSize).Take(batchSize);
                        records = new MessageProcessor(analyzer.instrumentid).PrepareWorklistMessages(new List<string> { "ALL" }, wList.samples.Skip(cntr).Take(batchSize), isCancellation);
                        SendWorkOrder(records);
                        UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Normal, "Written WO file for " + records.Count + " records.");
                    }
                }
        ...
        private void UpdateConnectivityStatus()
        {
            ...
            //else if (!WebAPI.IsVsoftConnectionSucceeded)
            ...
            if (message != "")
            {
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, message);
            }
        ...
        private void CheckThreadCount()
        {
            ...
            if (threadCount > 320)
            {
                UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Error, "ALERT: Consider restarting LIS, thread count reached " + threadCount);
            }
        }
        ...
        public static void ScheduleMethodInThread(Action MethodReference, TimeSpan ts)
        {
            new Thread(() =>
            {
                while (true)
                {
                    ...
                    {
                        UiMediator.LogAndShowError(analyzer.instrumentid, ex, "Error in " + nameof(MethodReference)/*, "Error in " + MethodReference.Method.Name*/);
                    }
        ...
        private void SendWorkOrder(List<string> records, string barcode = "")
        {
            if (records.Count > 0)
            {
                if (connectionSettings_WO.IsFileBased)
                {
                    ...
                    UiMediator.AddUiMessage(analyzer.instrumentid, (int)MessageType.Outgoing, "Written WO file to folder: " + woFilePath);
                }
                else
                {
